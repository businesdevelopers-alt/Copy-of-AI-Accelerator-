...
export const AssessmentResult: React.FC<AssessmentResultProps> = ({ result, onContinue }) => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-brand-primary flex flex-col items-center justify-center p-6 text-right" dir="rtl">
      <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        
        {/* Visual Analytics */}
        <div className="flex flex-col items-center animate-fade-in-up order-2 lg:order-1">
           <div className="mb-10 text-center">
              <span className="bg-brand-primary/20 text-white text-[10px] font-bold px-4 py-1.5 rounded-full border border-brand-primary/20 uppercase tracking-widest mb-4 inline-block">Competency Radar Map</span>
              <h2 className="text-3xl font-bold">تحليل الجاهزية الشامل</h2>
           </div>
           <div className="relative w-[340px] h-[340px] group">
             <div className="absolute inset-0 bg-brand-hover/5 rounded-full blur-[80px] -z-0"></div>
        ...
        {/* Decision Summary */}
        <div className="bg-brand-primary/50 p-10 md:p-14 rounded-[3.5rem] border border-brand-primary/10 animate-fade-in-up shadow-2xl relative overflow-hidden backdrop-blur-xl order-1 lg:order-2">
           ...
           <div className="flex justify-between items-center mb-10">
             <div className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Admission Score Verdict</div>
             <div className={`text-3xl font-bold ${result.isQualified ? 'text-green-400' : 'text-red-400'} tracking-tighter`}>
               {animatedScore}<span className="text-xl text-slate-600 ml-1">/100</span>
             </div>
           </div>

           <div className="mb-12 space-y-6">
             <div className={`inline-flex items-center gap-3 px-6 py-2 rounded-2xl font-bold text-xs uppercase tracking-widest
           ...
           <button 
             onClick={onContinue}
             className={`w-full py-6 rounded-[2rem] font-bold text-xl shadow-2xl transition-all transform hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-4
               ${result.isQualified 
                ? 'bg-brand-primary hover:bg-brand-hover text-white shadow-brand-primary/20' 
                : 'bg-red-600 hover:bg-red-700 text-brand-primary shadow-red-500/20'
               }`}
           >
