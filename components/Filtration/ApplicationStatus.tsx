...
export const ApplicationStatus: React.FC<ApplicationStatusProps> = ({ profile, result, onNext }) => {
  ...
  return (
    <div className="min-h-screen bg-brand-bg py-12 px-4 flex items-center justify-center font-sans">
      <div className="max-w-3xl w-full bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden animate-fade-in-up">
        <div className="bg-white p-8 text-brand-primary flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold">حالة الطلب</h2>
            <p className="text-brand-gray text-xs mt-1 uppercase tracking-widest">تتبع طلب ريادة الأعمال الخاص بك</p>
          </div>
          <div className="text-right">
            <span className="text-[10px] font-bold text-slate-500 block mb-1">رقم المرجع</span>
            <span className="font-mono text-brand-primary">BD-{Math.floor(1000 + Math.random() * 9000)}</span>
          </div>
        </div>
        ...
        <div className="p-8 md:p-12">
              ...
              {steps.map((step) => (
                <div key={step.id} className="flex flex-col items-center gap-2">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 z-10 transition-all duration-500
                    ${step.status === 'completed' ? 'bg-brand-primary border-white text-white shadow-lg' : 
                      step.status === 'failed' ? 'bg-red-500 border-white text-white' :
                      step.status === 'locked' ? 'bg-gray-100 border-white text-gray-300' : 'bg-white border-brand-primary text-brand-primary'}
                  `}>
                  ...
                  <div className="text-center">
                    <p className={`text-[10px] font-bold whitespace-nowrap ${step.status === 'completed' ? 'text-brand-primary' : 'text-brand-gray'}`}>
                      {step.label}
                    </p>
                    <p className="text-[8px] text-slate-600 font-bold">{step.date}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100 mb-8">
            <div className="flex items-center gap-4 mb-6">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg transform -rotate-3
                ${result.isQualified ? 'bg-green-500 text-brand-primary' : 'bg-red-500 text-white'}
              `}>
                {result.isQualified ? '✅' : '❌'}
              </div>
              <div>
            ...
            <div className="space-y-4">
               <div className="flex justify-between items-center py-3 border-b border-slate-200">
                 ...
                 <span className="text-sm text-slate-900 font-bold">{profile.codeName}</span>
               </div>
               <div className="flex justify-between items-center py-3 border-b border-slate-200">
                 <span className="text-sm text-slate-500 font-bold">نتيجة الاختبار:</span>
                 <span className={`text-sm font-bold ${result.score >= 65 ? 'text-green-600' : 'text-red-600'}`}>
                    {result.score}/100
                 </span>
               </div>
               <div className="flex justify-between items-center py-3">
                 <span className="text-sm text-slate-500 font-bold">القرار:</span>
                 <span className={`text-sm font-bold px-3 py-1 rounded-full ${result.isQualified ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <button 
              onClick={onNext}
              className="flex-1 bg-brand-primary hover:bg-brand-hover text-white py-4 rounded-2xl font-bold shadow-xl shadow-brand-primary/20 transition-all transform hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-2"
            >
