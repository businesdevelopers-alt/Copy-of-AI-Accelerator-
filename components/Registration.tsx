...
export const Registration: React.FC<RegistrationProps> = ({ onRegister, onStaffLogin }) => {
  ...
  return (
    <div className="min-h-screen flex bg-brand-bg font-sans text-brand-primary" dir="rtl">
      <style>{`
      ...
      {/* Side Brand Panel */}
      <div className="hidden lg:flex lg:w-1/3 relative bg-white flex-col justify-between p-12 text-brand-primary border-l border-brand-primary/10 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-10">
            <div className="w-12 h-12 bg-brand-primary rounded-2xl flex items-center justify-center border border-brand-primary/30 shadow-2xl">
              <svg className="w-7 h-7 text-brand-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" /></svg>
            </div>
            <span className="text-2xl font-bold uppercase tracking-tighter">AI Accelerator</span>
          </div>
          <h1 className="text-3xl font-bold leading-tight mb-6">مرحلة <br/><span className="text-brand-hover">التعاقد الذكي.</span></h1>
          <p className="text-lg text-brand-gray max-w-xs">نضمن لك بيئة قانونية واضحة من اليوم الأول لنركز معاً على بناء المنتج ونمو الشركة.</p>
        </div>
        
        <div className="relative z-10 space-y-6">
           <div className="bg-brand-primary/10 border border-brand-primary/20 p-6 rounded-3xl backdrop-blur-md">
              <h4 className="text-brand-primary font-bold text-xs uppercase tracking-widest mb-2">النظام القضائي</h4>
              <p className="text-[11px] text-slate-600 leading-relaxed">تخضع هذه الاتفاقية لأنظمة وقوانين المملكة العربية السعودية وتعتبر ملزمة للطرفين فور التوقيع الرقمي.</p>
           </div>
        </div>
      </div>

      {/* Main Registration Area */}
      <div className="w-full lg:w-2/3 flex flex-col p-6 md:p-12 overflow-y-auto bg-brand-bg">
        <div className="max-w-3xl w-full mx-auto animate-fade-in-up">
          <header className="mb-12">
            <h2 className="text-3xl font-bold mb-2">بوابة رائد الأعمال</h2>
            <p className="text-brand-gray">يرجى إكمال البيانات تمهيداً لتوقيع عقد الاحتضان الرسمي.</p>
          </header>

          <form onSubmit={handleSubmit} className="space-y-12 pb-20">
            {/* Section 1: User Data */}
            <div className="space-y-8">
               <h3 className="text-xl font-bold text-brand-hover flex items-center gap-3">
                  <span className="w-8 h-8 rounded-full bg-brand-primary/20 flex items-center justify-center text-sm">1</span>
                  البيانات الشخصية والمهنية
               </h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">الاسم الأول</label>
                  <input 
                    name="firstName"
                    className={`w-full p-4 bg-brand-primary/5 border rounded-2xl outline-none transition-all ${errors.firstName ? 'border-rose-500 bg-rose-500/5 animate-shake' : 'border-brand-primary/20 focus:border-brand-primary'}`}
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">اللقب (العائلة)</label>
                  <input 
                    name="lastName"
                    className={`w-full p-4 bg-brand-primary/5 border rounded-2xl outline-none transition-all ${errors.lastName ? 'border-rose-500 bg-rose-500/5 animate-shake' : 'border-brand-primary/20 focus:border-brand-primary'}`}
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">اسم المشروع / الشركة الناشئة</label>
                  <input 
                    name="startupName"
                    className={`w-full p-4 bg-brand-primary/5 border rounded-2xl outline-none transition-all ${errors.startupName ? 'border-rose-500 bg-rose-500/5 animate-shake' : 'border-brand-primary/20 focus:border-brand-primary'}`}
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">وصف الفكرة</label>
                  <textarea 
                    name="startupDescription"
                    className={`w-full h-32 p-4 bg-brand-primary/5 border rounded-2xl outline-none transition-all resize-none ${errors.startupDescription ? 'border-rose-500 bg-rose-500/5 animate-shake' : 'border-brand-primary/20 focus:border-brand-primary'}`}
                  ...
                  <button type="button" onClick={handleAnalyzeIdea} disabled={isAnalyzing} className="text-[10px] font-bold text-brand-hover hover:text-brand-primary uppercase tracking-widest mt-1">
                     {isAnalyzing ? 'جاري التحليل...' : '✨ تحليل ذكي مبدئي للفكرة (اختياري)'}
                  </button>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">البريد الإلكتروني الرسمي</label>
                  <input 
                    name="email"
                    type="email" 
                    className={`w-full p-4 bg-brand-primary/5 border rounded-2xl outline-none transition-all ${errors.email ? 'border-rose-500 bg-rose-500/5 animate-shake' : 'border-brand-primary/20 focus:border-brand-primary'}`}
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">رقم الجوال (السعودية)</label>
                  <input 
                    name="phone"
                    className={`w-full p-4 bg-brand-primary/5 border rounded-2xl outline-none transition-all ${errors.phone ? 'border-rose-500 bg-rose-500/5 animate-shake' : 'border-brand-primary/20 focus:border-brand-primary'}`}
            ...
            <div className="pt-10 border-t border-brand-primary/10 space-y-8">
               <h3 className="text-xl font-bold text-brand-hover flex items-center gap-3">
                  <span className="w-8 h-8 rounded-full bg-brand-primary/20 flex items-center justify-center text-sm">2</span>
                  عقد الاحتضان والتسريع الافتراضي
               </h3>
               
               <div className={`bg-white text-brand-primary rounded-[2.5rem] shadow-2xl relative overflow-hidden border transition-all ${errors.agreedToTerms || errors.agreedToContract || errors.signedContractName ? 'border-rose-500' : 'border-slate-200'}`}>
                  <div className="bg-brand-bg p-6 border-b border-slate-200 flex justify-between items-center">
                     <span className="font-bold text-brand-gray text-xs uppercase tracking-widest">Official Legal Document</span>
                     <span className="font-bold text-brand-primary text-xs">تاريخ العقد: {today}</span>
                  </div>

                  <div className="p-8 md:p-12 h-96 overflow-y-auto contract-scroll text-sm leading-loose text-brand-primary font-medium">
                     <div className="text-center mb-8">
                        <h4 className="text-2xl font-bold text-slate-900 underline underline-offset-8 decoration-blue-500">عقد احتضان وتسريع افتراضي</h4>
                  ...
                  <div className="p-8 md:p-12 border-t border-slate-100 bg-slate-50/50">
                     <div className="space-y-4 mb-8">
                        <div className="flex items-center gap-4">
                           ...
                           />
                           <label htmlFor="terms" className="text-sm font-bold text-slate-900 cursor-pointer select-none">أوافق على الشروط والأحكام العامة للمنصة وسياسة الخصوصية.</label>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                           ...
                           />
                           <label htmlFor="contract-agreement" className="text-sm font-bold text-slate-900 cursor-pointer select-none">أقر أنا الطرف الثاني بقبول كافة بنود عقد الاحتضان المذكور أعلاه والالتزام بها.</label>
                        </div>
                        {errors.agreedToContract && <p className="text-[10px] font-bold text-rose-500 pr-10">{errors.agreedToContract}</p>}
                     </div>

                     <div className="space-y-4">
                        <label className="block text-[10px] font-bold text-brand-gray uppercase tracking-[0.2em] pr-1">التوقيع الرقمي (اكتب اسمك الكامل للمطابقة)</label>
                        <div className="relative group">
                           <input 
                              name="signedContractName"
                              className={`w-full p-6 bg-white border-2 rounded-[1.5rem] outline-none text-2xl font-serif italic text-brand-primary placeholder-slate-200 transition-all shadow-inner
                                 ${errors.signedContractName ? 'border-rose-500 bg-rose-50 animate-shake' : 'border-slate-200 focus:border-brand-primary'}
                              `}
            ...
            {Object.keys(errors).length > 0 && (
               <div className="p-6 bg-rose-500/10 border border-rose-500/20 rounded-3xl animate-shake">
                  <p className="text-xs font-bold text-rose-500 mb-3 uppercase tracking-widest">يرجى تصحيح التنبيهات الموضحة في النموذج أعلاه</p>
            ...
            <button 
              type="submit" 
              className="w-full py-7 bg-brand-primary hover:bg-brand-hover text-white rounded-[2.2rem] font-bold text-xl shadow-2xl shadow-brand-primary/20 transition-all transform active:scale-95 flex items-center justify-center gap-4 group"
            >
      ...
      {analysisResult && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-white/90 backdrop-blur-md animate-fade-in" dir="rtl">
           <div className="bg-white rounded-[3rem] max-w-2xl w-full shadow-2xl border border-brand-primary/20 overflow-hidden animate-fade-in-up">
              <div className="bg-slate-100 p-8 md:p-10 border-b border-brand-primary/10 flex justify-between items-center">
                 <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-brand-primary rounded-2xl flex items-center justify-center text-2xl shadow-lg">✨</div>
                    <div>
                       <h3 className="text-2xl font-bold text-brand-primary">التحقق المبدئي من الفكرة</h3>
                       <p className="text-brand-primary text-[10px] font-bold uppercase tracking-widest mt-1">AI Validation Hub</p>
                    </div>
                 </div>
                 <button onClick={() => setAnalysisResult(null)} className="text-brand-gray hover:text-brand-primary transition-colors">✕</button>
              </div>

              <div className="p-8 md:p-10 space-y-8 overflow-y-auto max-h-[60vh] contract-scroll">
                 <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-brand-gray uppercase tracking-widest">درجة الجدوى الأولية</span>
                    <div className="text-3xl font-bold text-brand-hover">{analysisResult.totalScore}<span className="text-lg text-slate-600 ml-1">/100</span></div>
                 </div>

                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                       <h4 className="text-xs font-bold text-green-500 uppercase tracking-widest pr-2">نقاط القوة</h4>
                       <div className="space-y-2">
                    ...
                    <div className="space-y-4">
                       <h4 className="text-xs font-bold text-rose-500 uppercase tracking-widest pr-2">مخاطر محتملة</h4>
                       <div className="space-y-2">
                 ...
                 <div className="p-6 bg-brand-primary/5 border border-brand-primary/10 rounded-3xl relative group">
                    <div className="text-[10px] font-bold text-brand-primary uppercase tracking-widest mb-3">رأي المستشار الذكي</div>
                    <p className="text-sm text-slate-600 leading-relaxed font-medium italic">"{analysisResult.aiOpinion}"</p>
                 </div>
              </div>

              <div className="p-8 md:p-10 bg-slate-100/50 border-t border-brand-primary/10 flex flex-col gap-4">
                 <button 
                  onClick={() => setAnalysisResult(null)}
                  className="w-full py-5 bg-brand-primary hover:bg-brand-primary text-white rounded-2xl font-bold text-lg shadow-xl shadow-brand-primary/20 transition-all active:scale-95"
                 >
